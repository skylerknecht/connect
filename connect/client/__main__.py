...
# ToDo: Better banners so the results dont show up seperated
...
def agents(data):
    ...
    cli.notify('DEFAULT', '')
    cli.notify('DEFAULT',
               '{:<20}{:<20}{:<10}{:<30}{:<20}{:<20}{:<20}'.format('ID', 'Implant', 'Delta', 'Username', 'Hostname',
                                                                   'IP', 'OS'))
    cli.notify('DEFAULT',
               '{:<20}{:<20}{:<10}{:<30}{:<20}{:<20}{:<20}'.format('-' * 2, '-' * 7, '-' * 5, '-' * 8, '-' * 8, '-' * 2,
                                                                   '-' * 2))
    for index, agent in enumerate(agents):
        ...
        if delta < 60 or data['all'] == "True":
            cli.notify('DEFAULT', '{:<20}{:<20}{:<10}{:<30}{:<20}{:<20}{:<20}'.format(agent.name, agent.implant, delta,
...
def socks(data):
    socks = data['socks']
    cli.notify('DEFAULT', '{:<16}{:<7}{:<16}'.format('', '[SOCKS]', ''))
    cli.notify('DEFAULT', '')
    cli.notify('DEFAULT', '{:<5}{:<20}{:<15}'.format('ID', 'ENDPOINT', 'STATUS'))
    cli.notify('DEFAULT', '{:<5}{:<20}{:<15}'.format('-' * 2, '-' * 8, '-' * 6))
    for index, sock in enumerate(socks):
        socks[index] = output.SocksProxy(*sock)
        sock = socks[index]
        cli.notify('DEFAULT', '{:<5}{:<20}{:<15}'.format(sock.id, sock.ip_port, sock.status))
...
def main():
    ...
    parser = argparse.ArgumentParser('ConnectClient', 'python3 -m connect.client <team_server_uri> <key>',
